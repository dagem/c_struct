...
stack* alloc_stack()					//responsible for allocating memory for the stack.
{
	...
	st->top = alloc_st_node();				//allocates memory for the stack's top node.
	st->size = 0;					//sets the size of the stack to 0.
	return st;					//returns our stack node.
}
st_node* alloc_st_node()
{
	st_node* new_node = malloc(sizeof(st_node));		//allocates memory for a node.
...
{
	st_node* tmp = NULL;				//we create a node with no memory.
...
void push(stack *st, void* udata)
{
	...
	st_node* new_node = alloc_st_node();          //since we have a stack node, we add nodes "on top" of it.
...
void* pop(stack* st)
{
	if(st->size == 1)
	{
		...
		st_node* tmp = NULL;		//a temporary node used to "clear" the stack.
